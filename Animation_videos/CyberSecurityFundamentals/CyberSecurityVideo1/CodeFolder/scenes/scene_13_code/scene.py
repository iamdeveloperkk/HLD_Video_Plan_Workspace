...
from scenes.scene_13_code.config import *
...
def draw_header(image, elapsed):
    d = ImageDraw.Draw(image)
    d.text((50,35), "THE INVESTIGATION CONTINUES", font=font(26,True), fill=WHITE)
...
def make_frame(elapsed):
    ...
    cols=[65,330,595]; rows=[165,315]
    for i,(name,desc) in enumerate(CARDS):
        x=cols[i%3]; y=rows[i//3]
        b=Box(name,Rect(x,y,225,120),name,CARD_REVEALS[i],fill=(32,34,48),border=RED,sublabel=desc)
        b.draw(image,elapsed,active=(elapsed>=CARD_REVEALS[i]+0.8))
    if elapsed>=QUESTION_REVEAL:
        d.rounded_rectangle((100,455,785,505),radius=10,fill=rgba((65,35,40),230),outline=rgba(RED,230),width=2)
        centered_text(d,(115,462,770,496),"WHAT HAPPENS WHEN THESE ATTACKS MEET REAL SYSTEMS?",font(14,True),WHITE)
    if elapsed>=SERIES_REVEAL:
        centered_text(d,(120,535,770,580),"CYBER ATTACK X FAANG",font(25,True),BLUE)
    draw_panel(image,SCENE_TITLE,DISCUSSION_POINTS,DISCUSSION_REVEALS,elapsed)
    draw_takeaway(image,TAKEAWAY,SERIES_REVEAL,elapsed)
    return image
