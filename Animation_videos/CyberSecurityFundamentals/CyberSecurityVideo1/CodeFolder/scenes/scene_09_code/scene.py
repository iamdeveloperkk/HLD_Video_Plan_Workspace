...
from scenes.scene_09_code.config import *
...
def draw_header(image, elapsed):
    d = ImageDraw.Draw(image)
    d.text((50,35), "FINDING THE WEAKNESS", font=font(26,True), fill=WHITE)
...
def make_frame(elapsed):
    ...
    req=Box("request",Rect(*REQUEST_RECT),"REQUEST",REQUEST_REVEAL,sublabel="normal traffic")
    svc=Box("service",Rect(*SERVICE_RECT),"ORDER SERVICE",SERVICE_REVEAL,sublabel="business logic")
    data=Box("data",Rect(*DATA_RECT),"DATA",SERVICE_REVEAL,sublabel="state")
    req.draw(image,elapsed); svc.draw(image,elapsed); data.draw(image,elapsed)
    Line("r1",req.output(),svc.input(),REQUEST_REVEAL,3,BLUE).draw(image,elapsed)
    Line("r2",svc.output(),data.input(),SERVICE_REVEAL,3,BLUE).draw(image,elapsed)
    if elapsed>=WEAKNESS_REVEAL:
        d.rounded_rectangle((svc.x+25,svc.y+70,svc.bounds.right-25,svc.y+125),radius=8,fill=rgba((65,35,40),255),outline=rgba(RED,255),width=2)
        centered_text(d,(svc.x+35,svc.y+76,svc.bounds.right-35,svc.y+119),"WEAK AUTHORIZATION",font(13,True),WHITE)
    if elapsed>=BOUNDARY_REVEAL:
        d.line((svc.x+10,svc.y+10,svc.x+svc.width-10,svc.y+10),fill=rgba(RED,220),width=3)
        d.text((svc.x+20,svc.y+20),"SECURITY BOUNDARY",font=font(11,True),fill=rgba(RED,230))
