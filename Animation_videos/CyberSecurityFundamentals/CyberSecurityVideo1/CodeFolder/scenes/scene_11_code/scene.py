...
from scenes.scene_11_code.config import *
...
def draw_header(image, elapsed):
    d = ImageDraw.Draw(image)
    d.text((50,35), "SO HOW BAD IS IT?", font=font(26,True), fill=WHITE)
...
def make_frame(elapsed):
    ...
    x,y,w,h=MATRIX_RECT
    d.rounded_rectangle((x,y,x+w,y+h),radius=12,fill=rgba(PANEL,255),outline=rgba(BLUE,220),width=2)
    d.text((x+18,y+12),"RISK MATRIX",font=font(14,True),fill=WHITE)
    gridx=x+55; gridy=y+55; gw=w-80; gh=h-90
    for i in range(1,4):
        d.line((gridx,gridy+gh*i/3,gridx+gw,gridy+gh*i/3),fill=rgba(GRID,230),width=1)
        d.line((gridx+gw*i/3,gridy,gridx+gw*i/3,gridy+gh),fill=rgba(GRID,230),width=1)
    d.text((gridx+gw/2-35,gridy+gh+10),"LIKELIHOOD",font=font(11,True),fill=MUTED)
    d.text((x+10,gridy+gh/2-10),"IMPACT",font=font(11,True),fill=MUTED)
    if elapsed>=IMPACT_REVEAL:
        d.ellipse((gridx+gw*0.62-11,gridy+gh*0.20-11,gridx+gw*0.62+11,gridy+gh*0.20+11),fill=RED)
        d.text((gridx+gw*0.62+18,gridy+gh*0.20-9),"HIGH IMPACT",font=font(12,True),fill=WHITE)
    if elapsed>=LIKELIHOOD_REVEAL:
        d.ellipse((gridx+gw*0.72-9,gridy+gh*0.28-9,gridx+gw*0.72+9,gridy+gh*0.28+9),fill=YELLOW)
        d.text((gridx+gw*0.72+15,gridy+gh*0.28+4),"LIKELY",font=font(11,True),fill=WHITE)
    Meter("impact",Rect(*IMPACT_RECT),"IMPACT",IMPACT_REVEAL,0.9,RED).draw(image,elapsed,elapsed>=ESCALATION_REVEAL)
    Meter("likelihood",Rect(*LIKELIHOOD_RECT),"LIKELIHOOD",LIKELIHOOD_REVEAL,0.72,YELLOW).draw(image,elapsed,elapsed>=ESCALATION_REVEAL)
    if elapsed>=RISK_REVEAL:
        d.rounded_rectangle((RISK_RECT[0],RISK_RECT[1],RISK_RECT[0]+RISK_RECT[2],RISK_RECT[1]+RISK_RECT[3]),radius=8,fill=rgba(RED,240),outline=rgba(RED,255),width=2)
        centered_text(d,(RISK_RECT[0],RISK_RECT[1],RISK_RECT[0]+RISK_RECT[2],RISK_RECT[1]+RISK_RECT[3]),"RISK: HIGH",font(16,True),WHITE)
