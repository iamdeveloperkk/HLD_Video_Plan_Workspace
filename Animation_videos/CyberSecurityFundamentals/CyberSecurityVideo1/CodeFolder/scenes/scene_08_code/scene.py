...
from scenes.scene_08_code.config import *
...
def draw_header(image, elapsed):
    d = ImageDraw.Draw(image)
    d.text((50,35), "HOW DO THEY GET IN?", font=font(26,True), fill=WHITE)
...
def make_frame(elapsed):
    ...
    center=Box("target",Rect(*CENTER_RECT),"NEXA TARGET",CENTER_REVEAL,sublabel="protected system")
    center.draw(image,elapsed)
    cx,cy=center.x,center.y
    for i,(label,rect) in enumerate(PATHS):
        b=Box(label,Rect(*rect),label,PATH_REVEALS[i],fill=(30,40,60),border=BLUE)
        b.draw(image,elapsed)
        p1=b.output(); p2=center.input()
        # route through a midpoint to keep lines clean
        midx=(p1[0]+p2[0])/2
        Line(f"route{i}",p1,(midx,p1[1]),PATH_REVEALS[i],2,BLUE).draw(image,elapsed)
        Line(f"route{i}b",(midx,p1[1]),(midx,p2[1]),PATH_REVEALS[i]+0.15,2,BLUE).draw(image,elapsed)
        Line(f"route{i}c",(midx,p2[1]),p2,PATH_REVEALS[i]+0.3,2,BLUE).draw(image,elapsed)
    if elapsed>=CONVERGE_REVEAL:
        Dot("converge",cx,cy+center.height/2,CONVERGE_REVEAL,r=9,fill=RED).draw(image,elapsed,True)
