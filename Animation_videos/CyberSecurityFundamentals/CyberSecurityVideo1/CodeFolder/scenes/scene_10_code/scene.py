...
from scenes.scene_10_code.config import *
...
def draw_header(image, elapsed):
    d = ImageDraw.Draw(image)
    d.text((50,35), "VULNERABILITY BECOMES AN ATTACK", font=font(26,True), fill=WHITE)
...
def make_frame(elapsed):
    ...
    client=Box("client",Rect(*CLIENT_RECT),"CLIENT",NORMAL_REVEAL,sublabel="request")
    edge=Box("edge",Rect(*EDGE_RECT),"EDGE",NORMAL_REVEAL,sublabel="inspection")
    service=Box("service",Rect(*SERVICE_RECT),"SERVICE",NORMAL_REVEAL,sublabel="vulnerable path")
    data=Box("data",Rect(*DATA_RECT),"DATA",NORMAL_REVEAL,sublabel="state")
    for b in [client,edge,service,data]: b.draw(image,elapsed,active=(b is service and elapsed>=EXPLOIT_REVEAL))
    Line("a",client.output(),edge.input(),NORMAL_REVEAL,3,BLUE).draw(image,elapsed)
    Line("b",edge.output(),service.input(),NORMAL_REVEAL,3,BLUE).draw(image,elapsed)
    Line("c",service.output(),data.input(),NORMAL_REVEAL,3,BLUE).draw(image,elapsed)
    if elapsed>=BOUNDARY_REVEAL:
        d.line((edge.x+edge.width,edge.y+15,edge.x+edge.width,edge.bounds.bottom-15),fill=rgba(BLUE,220),width=3)
        d.text((edge.x+18,edge.y+18),"SECURITY CHECK",font=font(11,True),fill=rgba(BLUE,230))
    if elapsed>=ATTACK_REVEAL:
        Line("attack",(client.output()[0],client.output()[1]+12),(service.input()[0],service.input()[1]+12),ATTACK_REVEAL,6,RED).draw(image,elapsed)
        Dot("attackdot",service.x-5,service.y+service.height/2+12,ATTACK_REVEAL,r=9,fill=RED).draw(image,elapsed,True)
    if elapsed>=IMPACT_REVEAL:
        d.rounded_rectangle((service.x+20,service.y+145,service.bounds.right-20,service.y+185),radius=8,fill=rgba((65,35,40),240),outline=rgba(RED,240),width=2)
        centered_text(d,(service.x+25,service.y+150,service.bounds.right-25,service.y+180),"BOUNDARY BYPASSED",font(12,True),WHITE)
