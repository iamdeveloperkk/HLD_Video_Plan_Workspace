...
def draw_header(image, elapsed):
    d = ImageDraw.Draw(image)
    d.text((50,35), "WHO IS DOING THIS?", font=font(26,True), fill=WHITE)
...
def make_frame(elapsed):
    ...
    system=Box("system",Rect(*SYSTEM_RECT),"NEXA SYSTEM",SYSTEM_REVEAL,sublabel="protected environment")
    threat=Box("threat",Rect(*THREAT_RECT),"UNKNOWN ACTOR",THREAT_REVEAL,fill=(45,30,38),border=RED,sublabel="potential source of harm")
    signal=Line("signal",(305,302),(590,302),SIGNAL_REVEAL,3,RED)
    dot=Dot("signal_dot",450,302,SIGNAL_REVEAL,r=8,fill=RED)
    profile=Meter("capability",Rect(345,430,410,65),"THREAT SIGNAL",PROFILE_REVEAL,0.78,RED)
    for e in [signal,dot,system,threat,profile]: e.draw(image,elapsed,active=(e is threat and elapsed>8.6))
