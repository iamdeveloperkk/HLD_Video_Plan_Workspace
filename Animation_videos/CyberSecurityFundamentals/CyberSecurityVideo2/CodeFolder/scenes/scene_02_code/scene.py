...
from scenes.scene_02_code.config import *
...
def make_frame(elapsed):
    ...
    a=Box("server",Rect(75,245,190,110),"WEB APP",1.0,"renders page")
    b=Box("browser",Rect(640,245,190,110),"VICTIM BROWSER",2.2,"browser context")
    a.draw(image,elapsed); b.draw(image,elapsed); Arrow("q",a.output(),b.input(),2.2).draw(image,elapsed)
    Dot("content",a.output(),b.input(),3.2).draw(image,elapsed)
    if elapsed>=6:
        d.rounded_rectangle((300,205,570,385),radius=12,fill=rgba((65,35,40),235),outline=rgba(RED,240),width=2)
        centered_text(d,(315,220,555,265),"UNTRUSTED CONTENT",font(17,True),WHITE)
        centered_text(d,(315,275,555,345),"interpreted by browser",font(16),WHITE)
    if elapsed>=9: d.text((350,400),"ENCODING + CSP REDUCE RISK",font=font(14,True),fill=GREEN)
