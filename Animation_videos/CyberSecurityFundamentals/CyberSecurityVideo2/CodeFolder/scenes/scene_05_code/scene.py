...
def make_frame(elapsed):
    ...
    c=Box("client",Rect(70,245,180,110),"LOGIN CLIENT",1.0,"authentication")
    a=Box("auth",Rect(365,220,190,160),"AUTH SERVICE",2.0,"verifies credentials")
    r=Box("account",Rect(680,245,160,110),"ACCOUNT",3.0,"protected resource")
    for b in [c,a,r]: b.draw(image,elapsed)
    Arrow("a",c.output(),a.input(),2.0).draw(image,elapsed); Arrow("b",a.output(),r.input(),3.0).draw(image,elapsed)
    Dot("attempt",c.output(),a.input(),4.0,2.5).draw(image,elapsed)
    if elapsed>=6: d.text((300,440),"AUTOMATED LOGIN ATTEMPTS",font=font(15,True),fill=RED)
    if elapsed>=9: d.text((275,475),"MFA + THROTTLING + DETECTION",font=font(14,True),fill=GREEN)
