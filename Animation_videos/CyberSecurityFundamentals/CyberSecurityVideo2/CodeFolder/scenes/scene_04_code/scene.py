...
def make_frame(elapsed):
    ...
    edge=Box("edge",Rect(340,285,170,110),"EDGE / WAF",2.0,"filters traffic")
    svc=Box("service",Rect(650,285,180,110),"SERVICE",3.0,"limited capacity")
    edge.draw(image,elapsed); svc.draw(image,elapsed); Arrow("e",edge.output(),svc.input(),3.0).draw(image,elapsed)
    for i,(x,y) in enumerate([(70,180),(70,280),(70,380),(70,480)]):
        s=Box(f"s{i}",Rect(x,y,155,62),"TRAFFIC SOURCE",1.0+i*.6,"distributed",RED)
        s.draw(image,elapsed); Arrow(f"x{i}",s.output(),edge.input(),4.0,RED,3).draw(image,elapsed)
    if elapsed>=5.2: d.text((350,210),"TRAFFIC FLOOD",font=font(16,True),fill=RED)
    if elapsed>=8: d.text((555,440),"LATENCY ↑   QUEUE ↑",font=font(15,True),fill=RED)
