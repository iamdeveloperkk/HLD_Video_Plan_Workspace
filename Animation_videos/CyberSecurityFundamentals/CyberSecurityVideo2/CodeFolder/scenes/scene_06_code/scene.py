...
def make_frame(elapsed):
    ...
    c=Box("client",Rect(70,245,180,110),"CLIENT",1.0,"expects TLS")
    m=Box("middle",Rect(360,215,210,170),"INTERMEDIARY",3.0,"untrusted position",RED,(45,30,38))
    s=Box("server",Rect(680,245,160,110),"SERVER",1.5,"trusted endpoint")
    c.draw(image,elapsed); m.draw(image,elapsed,elapsed>=6); s.draw(image,elapsed)
    Arrow("a",c.output(),m.input(),4.0,RED).draw(image,elapsed); Arrow("b",m.output(),s.input(),5.0,RED).draw(image,elapsed)
    if elapsed>=8: d.text((330,440),"TLS + CERTIFICATE VALIDATION",font=font(14,True),fill=GREEN)
